...
> 1. Install the app from this listing.
> 2. You'll land in the embedded app inside Shopify Admin.
> 3. Click "Start free trial" to activate billing (test mode on dev stores — no real charge).
> 4. The "What's next" card shows what's coming in Phase 2; current functionality is the install + billing + shop-info loop. Phase 2 features (product sync UI, post composer, FB/IG scheduling) ship in subsequent releases.

## Test credentials for reviewer (if anything beyond install)

_(None required — install is the test. Token Exchange + Managed Install means the reviewer just clicks Install.)_
