...
7. imagePrompt: Read this post's topic and content FIRST, then write a scene that VISUALLY REPRESENTS what this specific post is communicating — the image must match the post's TOPIC and EMOTIONAL MESSAGE, not just the business category. Apply this topic-to-scene mapping:
   • Pain Points / Problem / "wasted time" posts → something that shows the frustration or the problem: overflowing notebook, desk covered in sticky notes, alarm clock, crammed schedule planner, a cluttered chaotic surface
   • Solution / Outcome / Success / "what if" posts → something that shows clarity and resolution: clean minimal surface, single well-placed tool, tidy organised workspace
   • Educational / Tips / How-to posts → a concept-in-action scene: measuring tape, stopwatch, checklist mid-way through, one tool being used
   • Product / Feature / Promo posts → the specific product or its context of use named directly
   • Behind the Scenes / Process posts → a mid-action shot of the work in progress
   Allowed compositions (pick the ONE that best fits this post's topic): ${filterImagePromptExamples(getImagePromptExamples(effectiveBusinessType), safeProfile?.forbiddenSubjects)}. If no preset matches the post's topic well, describe a closely related scene from the same business environment that DOES communicate this post's message. Format: "[specific scene matching this post's message] on [surface], [bright natural lighting], [camera angle]". NEVER name a generic "product" or "items" — name the SPECIFIC THING. NEVER include people, hands, faces. ${bd.imagePromptAvoid}
