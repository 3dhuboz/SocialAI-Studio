 ...
 */
export const generateMarketingImageUrl = async (prompt: string, businessType: string = 'small business', caption?: string | null, clientId?: string | null, seedHint?: string | null): Promise<string | null> => {
...
7a. CAMPAIGN VISUAL SUBJECT RULE — when an active campaign mentions a website, URL, launch page, online ticket sales, bookings, or "push ticket sales", treat that website as the SALES CHANNEL, not the image subject. For non-tech businesses, NEVER depict a laptop, website screen, browser, desk, office planner, corkboard, notebook, content calendar, generic workspace, tickets, wristbands, or passes for those campaign posts. Depict the real-world thing being sold or promoted instead: the venue, entrance gate without signage, BBQ smoker, brisket/ribs on butcher paper, food truck row, trophies, market stall setup, or other on-archetype physical scene.
7aa. IMAGE TEXT KILL-SWITCH — NEVER ask the image model to render readable text. Do not request banners, signs, signage, posters, wristbands, tickets with writing, menus, labels, logos, headlines, brand names, event names, website text, prices, or any words/letters in the image. Image models misspell text and that is a hard customer-facing failure. Use text-free food/venue props instead: "festival banner saying Gladstone BBQ Festival" becomes "sliced brisket and ribs beside BBQ tongs"; "ticket price poster" becomes "smoked brisket trays and ribs on butcher paper".
   ...
   • Solution / Outcome / Success / "what if" posts → clarity and resolution (people-free): clean minimal surface, single well-placed tool, tidy organised workspace
   • Educational / Tips / How-to posts → concept-in-action object scene (NO HANDS): measuring tape mid-stretch on a board, stopwatch resting on a notebook, checklist mid-way through with a pen beside it, one tool propped mid-use
   • Product / Feature / Promo posts → the specific product or its context of use named directly, with NO ONE holding it
   • Behind the Scenes / Process posts → mid-action shot of TOOLS or OBJECTS in motion, NO hands visible
   Allowed compositions (pick the ONE that best fits this post's topic): ${filterImagePromptExamples(getImagePromptExamples(effectiveBusinessType), safeProfile?.forbiddenSubjects)}. If no preset matches the post's topic well, describe a closely related people-free scene from the same business environment that DOES communicate this post's message. Format: "[specific people-free scene matching this post's message] on [surface], [bright natural lighting], [camera angle]". NEVER name a generic "product" or "items" — name the SPECIFIC THING. ${bd.imagePromptAvoid}
7b. VISUAL VARIETY MANDATE — across this batch of ${postsToGenerate} posts, NO TWO imagePrompts may share the same composition, subject framing, or setting. Rotate through DIFFERENT camera angles (overhead, side, macro, wide, action), DIFFERENT subjects (single item, group, environment, detail, abstract), and DIFFERENT lighting (DEFAULT to bright daylight; only pick golden hour / moody / soft window when the post tone explicitly calls for it). If you catch yourself reaching for the same fallback (notepad on a desk, laptop on a desk, coffee cup beside a planner, generic workspace flatlay) for ANY post in this batch, STOP and pick a totally different scene that reflects THIS POST'S SPECIFIC TOPIC — outdoor, in-situ, in-action, concept-driven.
...
Respond with ONLY a valid JSON object — no markdown, no code fences:
{
  ...
  "posts": [
    ...
    const processOne = async (p: any) => {
      ...
      // Defense-in-depth: the prompt template forbids people-in-scene, but
      // the bulk LLM occasionally slips "happy customers / families enjoying /
      // queue of people" into imagePrompt. Downstream the cron's
      // buildSafeImagePrompt strips PEOPLE_REGEX matches anyway, but doing it
      // here too means the UI/D1 see the same prompt FLUX will receive AND
      // we log how often the rule is being disobeyed. Cleanup-only — no
      // auto-recovery trigger, because the stripped prompt is usually still
      // good enough for the cron's archetype-aware fallback to anchor on.
      if (typeof p.imagePrompt === 'string') {
        const cleaned = p.imagePrompt.replace(PEOPLE_REGEX, '').replace(/\s+/g, ' ').trim();
    ...
    // Collision-spread: sort posts by scheduledFor, then walk the list and
    // advance any post that sits within 1 hour of the preceding one. Fixes
    // the "three posts all bumped to Tuesday 09:00" failure mode that the
    ...
    posts.sort((a, b) => {
      ...
      return new Date(a.scheduledFor.replace('Z', '')).getTime()
           - new Date(b.scheduledFor.replace('Z', '')).getTime();
    });
    for (let i = 1; i < posts.length; i++) {
      const prev = posts[i - 1];
      const curr = posts[i];
      if (!prev.scheduledFor || !curr.scheduledFor) continue;
      const prevT = new Date(prev.scheduledFor.replace('Z', '')).getTime();
