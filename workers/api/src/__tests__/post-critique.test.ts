...
describe('post critique helpers', () => {
  ...
  it('always includes the exact image brief even when the caption is detailed', () => {
    const context = buildCritiqueContextText({
    ...
    expect(context).toContain('Intended image brief: Close-up of bark and smoke ring');
  });
