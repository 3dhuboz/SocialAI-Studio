...
- GENERIC-SUBSTITUTE RULE (revised 2026-05-19): Two trigger forms — both score 3, match="partial".
  FORM A (specific-feature trigger): Caption names a specific product feature, dashboard, screen, or capability AND the image is a generic substitute scene (closed laptop, blank notebook with pen, unattended coffee cup, abstract gradient, plain keyboard, empty workspace) that does NOT depict the named feature. Reasoning MUST name the missing feature, e.g. "caption promises multi-client dashboard, image shows generic closed laptop with no UI visible".
  FORM B (image-led trigger — load-bearing): Caption is a tech/SaaS/agency/automation pitch (mentions any of: autopilot, set it up once, posts go out automatically, AI writes captions, AI generates images, schedules posts, content scheduling, marketing automation, agency dashboard, multi-client, white-label, hands-free posting, "we built X", "our platform/tool/app") AND the image is ANY recognizable generic-substitute flatlay scene: notebook with stationery (pen, sticky notes, paperclips, highlighters), closed laptop on desk, laptop with generic code at night, coffee cup + planner, sticky notes spread on desk, blank workspace, plain keyboard, abstract gradient, smartphone face-down on table, or office desk with succulent + mug. FORM B fires even when the image is on-archetype for tech-saas-agency — a notebook flatlay is "on-archetype" but the whole point is that on-archetype-but-generic is the failure mode we want to fail. This rule fires REGARDLESS of whether the caption named a specific feature — most marketing copy is discursive/vibe-y and doesn't name features explicitly. Reasoning MUST name BOTH the SaaS-pitch phrase that triggered it AND the generic-substitute scene pattern, e.g. "caption is a 'set it up once' SaaS pitch promising autopilot posting, image is a generic notebook-with-sticky-notes flatlay that depicts nothing specific to the product". The image is the load-bearing signal — there are only ~10 generic-substitute scene types and the vision model can recognize them reliably; captions are too varied to gate on. False-positive guard: only fire when archetype is tech-saas-agency OR the caption explicitly mentions a software/automation pitch — a cafe-archetype "behind the scenes — here's my notebook of ideas" post legitimately on-topic with a notebook image should NOT trip.
