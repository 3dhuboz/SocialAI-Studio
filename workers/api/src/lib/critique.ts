...
- GENERIC-SUBSTITUTE RULE (added 2026-05-19): Caption mentions a SPECIFIC named product, feature, dashboard, screen, or capability AND the image is a generic substitute scene — a closed laptop on a desk, a blank notebook with a pen, an empty workspace, an unattended coffee cup, an abstract gradient, or a plain keyboard — that does NOT depict the named feature → score 3, match="partial". Reasoning MUST name what specific feature/product the caption promised and what the image showed instead (e.g. "caption promises multi-client dashboard, image shows generic closed laptop with no UI visible"). This rule fires even when the image is on-archetype (a laptop on a SaaS post still fails when the caption named a specific dashboard feature) — the gate is "did the image show what the caption said it would show".
