...
export async function cronRefreshFacts(env: Env): Promise<{ posts_processed: number }> {
  ...
  // ── Shopify shops ─────────────────────────────────────────────────────
  // Phase 2+. Same about/posts/photos scrape shape but scoped by
  // shop_domain into shopify_facts (separate table from client_facts).
  // Skips uninstalled shops via the IS NULL guard. Shopify shops don't
  // have a Postproxy path — Postproxy is for the legacy users/clients
  // tier only.
  ...
  console.log(`[CRON facts] refreshed ${processed} workspaces (${(legacyUsers.results || []).length + (legacyClients.results || []).length} legacy + ${(ppUsers.results || []).length + (ppClients.results || []).length} postproxy + ${(shops.results || []).length} shops)`);
